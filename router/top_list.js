...
router.get("/", (req, res) => {
  ...
  createWebAPIRequest(
    ...
    music_req => {
      ...
      var obj = {}
      if(type==2){
        console.log(typeof(music_req))
        obj = music_req.playlist.tracks.slice(0, 3);

      } else {
        obj.coverImgUrl = music_req.playlist.coverImgUrl
        obj.creator = music_req.playlist.creator
        obj.name = music_req.playlist.name
        obj.tags = music_req.playlist.tags
        obj.description = music_req.playlist.description
        obj.tracks = music_req.playlist.tracks.slice(0, 30)
      }
      res.send(obj);
    },
